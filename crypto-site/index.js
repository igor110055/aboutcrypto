...
function f(){
    ...
    const y = glossaryArray.map((x, index) =>{
       return(
        newArray.push( 
        `<div class="card ${index % 2 != 0? "bg-light": "" }">
            <div class="card-header" id="crypto">
                <h3><button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#a${index}">${x.term}</button> </h3>
            </div>
            <div class="collapse" id="a${index}" data-parent="#collapseSection">
                <div class="card-body">
                    <p class="card-text text-primary" >${x.definition}
                    </p>
                </div>
            </div>
    </div>` ))})
